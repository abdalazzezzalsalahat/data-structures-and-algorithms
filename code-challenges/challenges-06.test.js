...
const sortByChildren = (charArray) => {
  charArray.sort((frst,sec)=>{
    if(frst.children.length === sec.children.length){
      return frst.house - sec.house;
    }
    return frst.children.length - sec.children.length;
  });
  return charArray;
};
...
const hasChildrenValues = (arr, character) => {
  let flag = false;
  arr.forEach(element => {
    let childs = Object.values(element);
    if (childs[0] === character){
      flag = true;
      console.log(flag);
    }
  });
...
const hasChildrenEntries = (arr, character) => {
  ...
  //   let childs = Object.values(element);
  //   (childs[0] === character) ? flag = true : flag = false;
  // });
  // return flag;
};
