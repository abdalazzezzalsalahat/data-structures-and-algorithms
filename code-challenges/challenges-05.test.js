...
const sortStarWarsCharacters = (starWarsArr) => {
  starWarsArr.sort((frst,sec) => sec.height - frst.height);
  return starWarsArr;
};
...
const joinArray = (arr) => {
  let str = '';
  arr.forEach((elmnt)=> str = str +' '+ `${elmnt}`);
  return str.trim();
};
...
const listFoods = (recipe) => {
  let result = [];
  recipe.ingredients.forEach((elmnt)=>{
    let spceIndx = elmnt.indexOf(' ');
    let food = elmnt.slice(spceIndx+1);
    spceIndx = food.indexOf(' ');
    result.push(food.slice(spceIndx+1));
  });
  return result;
};
